...
### Added
- **`tool()` UI/transcript display shaping** — `display()`/`displayInput()`/`displayError()` (config: `toDisplayOutput`/`toDisplayInput`/`toDisplayError`) reshape a tool's output/input/errors for `streamHooks.onToolCall`/`onToolResult`/`onApproval`/`onSuspended`, independent of `transform()`/`toModelOutput` which still shapes what the model sees. `ToolResult.display` carries the shaped value; the model feedback loop and `suspendedRunStore` persistence are unaffected. Fixes a bug where `AgenticRunner`'s `adaptTool()` silently dropped `suspendSchema`/`resumeSchema`/`requireApproval` when normalizing a plain-object tool into the registry.
