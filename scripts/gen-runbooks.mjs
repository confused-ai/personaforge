...
function guideExample(guide, primarySym, spec) {
  ...
  if (!best) return null;
  // Return the full example — truncating mid-code produces invalid TypeScript
  // that
...
// llms-full.txt — full-content document for LLM crawlers (llmstxt.org / agentic crawlers)
const FULL_OUT = join(root, 'docs', 'llms-full.txt');
const mdToText = (md) =>
  md
    .replace(/^---\n[\s\S]*?---\n/, '')                // frontmatter
    .replace(/```(?:ts|typescript|bash|js|json)\n/g, '```\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();

let full = `# ${NAME} — Full Documentation\n\n`;
full += `> This file is the complete, machine-readable documentation for ${NAME} (${pkg.version}). `;
full += `It is the concatenation of the guides and runbooks below, suitable for agentic crawlers and RAG ingestion.\n\n`;
full += `_Generated by \`node scripts/gen-runbooks.mjs\`. Do not edit by hand._\n\n`;
full += `## Table of contents\n\n`;
for (const g of guides.sort()) full += `- ${g}\n`;
for (const e of entries) full += `- runbooks/${e.slug}\n`;
full += `\n---\n\n`;
...
for (const g of guides.sort()) {
  const content = mdToText(readMd(join(GUIDE_DIR, `${g}.md`)));
  if (!content) continue;
  full += `\n\n# Guide: ${g}\n\n${content}\n`;
}

for (const e of entries) {
  const content = mdToText(readMd(join(OUT, `${e.slug}.md`)));
  if (!content) continue;
  full += `\n\n# Runbook: ${e.slug}\n\n${content}\n`;
}
writeFileSync(FULL_OUT, full);

console.log(`runbooks: ${written} → docs/runbooks/  ·  index + llms.txt + llms-full.txt written  ·  ${entries.length} features`);
